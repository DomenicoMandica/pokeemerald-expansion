...
static struct Weaknesses
{
	u6 type;
	int count = 0;
};

static Weaknesses[18] GetWeaknessDict()
{
	Weaknesses[18] weakness;
	weakness[0].type = TYPE_BUG;
	weakness[1].type = TYPE_STEEL;
	weakness[2].type = TYPE_GROUND;
	weakness[3].type = TYPE_ROCK;
	weakness[4].type = TYPE_GRASS;
	weakness[5].type = TYPE_DARK;
	weakness[6].type = TYPE_FIGHTING;
	weakness[7].type = TYPE_ELECTRIC;
	weakness[8].type = TYPE_WATER;
	weakness[9].type = TYPE_FLYING;
	weakness[10].type = TYPE_POISON;
	weakness[11].type = TYPE_ICE;
	weakness[12].type = TYPE_GHOST;
	weakness[13].type = TYPE_PSYCHIC;
	weakness[14].type = TYPE_FIRE;
	weakness[15].type = TYPE_DRAGON;
	weakness[16].type = TYPE_NORMAL;
	weakness[17].type = TYPE_FAIRY;

	retuurn weakness;
}

static u16[] GetCounterToTeam(struct Pokemon* targetParty)
{
	
	// store species of player team
	u16[6] monData;
	u16[6] counterTeam;
	int i = 0;
	for (i = 0; i < 6; i++)
	{
		if (targetParty[i] != null)
		{
			monData[i] = GetMonData(targetParty[i], MON_DATA_SPECIES);
		}
	...
	Weaknesses[18] weaknesses = GetWeaknessDict(); // holds our teams weakeness
	...
	for (i = 0; i < 6; i++)
	{
		...
		if(monData != 0)
		{ 
			u8 defType1 = gSpeciesInfo[monData[i]].type[0];
			u8 defType2 = gSpeciesInfo[monData[i]].type[1];
		}
		else
		{
			continue;
	...
	// loop through every type in the game to setup weakness dict
	for (i = 0; i < 17; i++)
	{
		u8 atkType1 = i;
		u8 atkType2 = i;

		u16 typeEffectiveness *= GetTypeModifier(atkType1, defType1);
		...
		// here we figure out whats most effective against our mon

		if (typeEffectiveness > 1.0)
		{
			if (typeEffectiveness >= 4.0)
			{
				weaknesses[defType1].count += 2; // defending type is double weak
			}
			else
			{
				weaknesses[defType1].count++; // defending type is weak
			}
			...
			if (defType2 != defType1)
			{
				if (typeEffectiveness >= 4.0)
				{
					weaknesses[defType2].count += 2; // defending type is double weak
				}
				else
				{
					weaknesses[defType2].count++; // defending type is weak
				}
			}
		}
	}

	// loop through every poke in the game i guess
	double threshold = 2.0; // needs this to be classified as worth using
	int MAX_MON = 151;
	u16[MAX_MON] toRet;
		// for now just gen 1 
	for (int j = 1; j < MAX_MON; j++)
	{
		u8 type1 = gSpeciesInfo[j].type[0];
		u8 type2 = gSpeciesInfo[j].type[1];

		if (weaknesses[type1].count > threshold || weakness[type2].count > threshold)
		{
			// store mon id
			toRest[j - 1] = j;
		}
...
u8 CreateNPCTrainerPartyFromTrainer(struct Pokemon* party, const struct Trainer* trainer, bool32 firstTrainer, u32 battleTypeFlags)
{
		...
		| BATTLE_TYPE_TRAINER_HILL)))
	{
		...
		for (i = 0; i < monsCount; i++)
		{
			...
			case 0:
			{
				// To test, give everyone a bulbasaur

				const struct TrainerMonNoItemDefaultMoves* partyData = trainer->party.NoItemDefaultMoves;
				u16[] potentialMon = GetCounterToTeam(&gPlayerParty);
				fixedIV = partyData[i].iv * MAX_PER_STAT_IVS / 255;
				CreateMon(&party[i], potentialMon[i], partyData[i].lvl, fixedIV, TRUE, personalityValue, OT_ID_RANDOM_NO_SHINY, 0);
				break;
			}
